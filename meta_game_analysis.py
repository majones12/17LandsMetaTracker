...
def get_table_values(table_html: BeautifulSoup) -> dict:
    ...
    for row in rows_list:
        cells = row.find_all('td')
        if cells[0].string in ['Four-color','Four-color + Splash','Five-color']:
            #print(f" Added: {cells[0]}")
            record[cells[0].string] = (int(cells[1].string),
                                       int(cells[2].string))
    return record

def scrape(selected_expansion: str, selected_format: str, selected_group: str, new_from: any = False) -> pd.DataFrame:
    ...
    try:
        from_date_in = WebDriverWait(driver,10).until(
            EC.element_to_be_clickable((By.XPATH, '//*[@id="app"]/div/div[1]/div[4]/div/div[1]/div/input')))
        to_date_in = WebDriverWait(driver,10).until(
            EC.element_to_be_clickable((By.XPATH, '//*[@id="app"]/div/div[1]/div[4]/div/div[3]/div/input')))
    ...
    from_date = datetime.strptime(from_date_in.get_attribute('value'), '%m/%d/%Y')
    ...
    # so when scraping, when get an empty table - stop processing (mark as done or ...)
    try:
    ...
    while (cur_date <= to_date) and data_exists:
        ...
        if not cur_table: # current table is empty, so no new games from this point
...
if __name__ == '__main__':
    ...
    try:
        ...
        from_date_in = WebDriverWait(driver,10).until(
            EC.presence_of_element_located((By.XPATH, '//*[@id="app"]/div/div[1]/div[4]/div/div[1]/div/input')))
        to_date_in = WebDriverWait(driver,10).until(
            EC.presence_of_element_located((By.XPATH, '//*[@id="app"]/div/div[1]/div[4]/div/div[3]/div/input')))
    except Exception as e:
        print(e)
    ...
    user_groups = [x.get_attribute('value') for x in users.options] # All Users = ''
    ...
    if os.path.exists(filename):
        ...
        if (data['date'].max() < datetime.today()) and (selected_expansion == cur_expansion):
            print('Updating the data with latest.')
            updated_data = scrape(selected_expansion, selected_format, selected_group, 
                                  new_from=data['date'].max() + timedelta(days=1))
    ...
    deck_map = {'Mono-White':'Mono', 'Mono-Blue':'Mono', 'Mono-Black':'Mono', 'Mono-Red':'Mono', 'Mono-Green':'Mono',
       'Mono-White + Splash':'Mono', 'Mono-Blue + Splash':'Mono', 'Mono-Black + Splash':'Mono',
       'Mono-Red + Splash':'Mono', 'Mono-Green + Splash':'Mono', 'Azorius (WU)':'WU',
       'Dimir (UB)':'UB', 'Rakdos (BR)':'BR', 'Gruul (RG)':'RG', 'Selesnya (GW)':'GW',
       'Orzhov (WB)':'WB', 'Golgari (BG)':'BG', 'Simic (GU)':'GU', 'Izzet (UR)':'UR',
       'Boros (RW)':'RW', 'Azorius (WU) + Splash':'WU', 'Dimir (UB) + Splash':'UB',
       'Rakdos (BR) + Splash':'BR', 'Gruul (RG) + Splash':'RG',
       'Selesnya (GW) + Splash':'GW', 'Orzhov (WB) + Splash':'WB',
       'Golgari (BG) + Splash':'BG', 'Simic (GU) + Splash':'GU',
       'Izzet (UR) + Splash':'UR', 'Boros (RW) + Splash':'RW', 'Jeskai (WUR)':'Other',
       'Sultai (UBG)':'Other', 'Mardu (BRW)':'Other', 'Temur (RGU)':'Other', 'Abzan (GWB)':'Other',
       'Esper (WUB)':'Other', 'Grixis (UBR)':'Other', 'Jund (BRG)':'Other', 'Naya (RGW)':'Other',
       'Bant (GWU)':'Other', 'Jeskai (WUR) + Splash':'Other', 'Sultai (UBG) + Splash':'Other',
       'Mardu (BRW) + Splash':'Other', 'Temur (RGU) + Splash':'Other',
       'Abzan (GWB) + Splash':'Other', 'Esper (WUB) + Splash':'Other',
       'Grixis (UBR) + Splash':'Other', 'Jund (BRG) + Splash':'Other',
       'Naya (RGW) + Splash':'Other', 'Bant (GWU) + Splash':'Other', 'Four-color':'Other',
    ...
    deck_list = ['Mono','BR','BG','WB','UB','WU','RW','GW','UR','GU','RG','Other']
    merged = pd.DataFrame()
    for deck in deck_list:
        ...
        merged = merged.merge(temp_df, left_index=True, right_index=True, how='outer')

    # re-order the columns based on means of each
    merged = merged.reindex(merged.mean().sort_values(ascending=False).index, axis=1)
    # re-sort colors based on new order
    decks = ['Mono','BG','BR','WB','UB','WU','RW','GW','UR','GU','RG','Other']
    colors = ['#000000','#204523','#472728','#818282','#326080','#99bee8','#f0787a','#7cf078',
              '#6102a1','#02a199','#73541c','#ff9900']
    color_lookup = dict(zip(decks,colors))
    plot_colors = [color_lookup[x] for x in merged.columns]
    line_styles = ['-','--','-.',':','-','--','-.',':','-','--','-.',':']
    plt.rcParams['figure.figsize'] = [11,8]
    plt.rcParams['figure.autolayout'] = True
    merged.plot(title=f'{selected_expansion} {selected_format} {selected_group}', 
                ylabel='% of games', color=plot_colors, style=line_styles)
